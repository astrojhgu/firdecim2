...
pub fn main() {
    ...
    while omega < std::f64::consts::PI {
        ...
        let output_pwr=output.iter().map(|&x| (x as i32 * x as i32) as u64).sum::<u64>() as f64 / output.len() as f64;
        let output_pwr_db=10.0*output_pwr.log10();
        println!("{} {}", omega/std::f64::consts::PI, output_pwr_db);
    }
}
